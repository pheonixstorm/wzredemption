...

{
	...
	hResult = psBack->lpVtbl->Lock(psBack,NULL, &ddsd,DDLOCK_WAIT | DDLOCK_SURFACEMEMORYPTR | DDLOCK_WRITEONLY, NULL);

	if ( hResult != D3D_OK )
	{
		ASSERT((FALSE,"_bank_off_4101: lock back surface failed\n%s",DDErrorToString(hResult)));
	}

	psRendSurface->buffer = (unsigned char *) ddsd.lpSurface;
...

{
	...
	// unlock surface (must be done before flip) (sk)

	hResult = psBack->lpVtbl->Unlock(psBack,NULL);

	if ( hResult != D3D_OK )
	{
		ASSERT((FALSE,"_bank_on_4101: unlock back surface failed\n%s",DDErrorToString(hResult)));
	}
...
static void _thline_4101(int x1, int x2, int y, fixed btx1, fixed btx2, fixed bty1, fixed bty2, int xshift, iBitmap *tex)
...
static void _tthline_4101(int x1, int x2, int y, fixed btx1, fixed btx2, fixed bty1, fixed bty2, int xshift, iBitmap *tex)
